...
def find_display_apoz(apoz_obj, valid_loader, criterion, args):
    apoz_obj.register()
    ...
    for n, p in zip(apoz_obj.layer_names, apoz):
...
def trim_network(args, valid_loader, criterion, select_rate, itr):
    ...
    apoz_obj = APoZ(model)
    apoz = find_display_apoz(apoz_obj, valid_loader, criterion, args)
    # Masking
    mask = []
    ...
    # update the apoz model and cfgs
    apoz_obj.set_model_and_cfgs(model)
    # display the pruned model
    summary(model, (3, 32, 32))
    new_apoz = find_display_apoz(apoz_obj, valid_loader, criterion, args)
